...
#[tokio::test]
async fn console_errors_respects_ring_limit_and_truncation() {
    let mut state = new_state(0);

    // 60 error entries → ring keeps last 50
    for i in 0..60 {
    ...
    // Add one long error to verify truncation (len 1000)
    state.push_console_error(webai_core::ConsoleEntry {
        kind: "console-error".into(),
        level: "error".into(),
    ...
    // Ensure at least one error message is truncated to 500 chars
